...
document.addEventListener("DOMContentLoaded", function() {
    var videoboxes = document.querySelectorAll(".videobox");
    ...
    videoboxes.forEach(function(videobox) {
        ...
        videobox.addEventListener("mouseleave", function() {
            ...
            if (video) {
                video.pause();
                video.currentTime = 0; // Перемотка на первый кадр
            }
        });
    });
});
 ...
 document.addEventListener('DOMContentLoaded', function() {
    ...
    const readMoreButtons = document.querySelectorAll('.read-more a');

    readMoreButtons.forEach(button => {
      button.addEventListener('click', function(event) {
        event.preventDefault(); // Отменяем стандартное поведение ссылки

        // Получаем ID из атрибута data-target у кнопки
        const targetId = this.getAttribute('data-target');

        // Находим дополнительный блок по ID
        const extraContent = document.getElementById(targetId);
        
        if (extraContent) {
          // Проверяем текущий стиль display и переключаем его
          if (extraContent.style.display === 'block') {
            extraContent.style.transition = 'opacity 0.5s';
    ...
    // Закрытие блока по ссылке внутри extra-content
    const closeButtons = document.querySelectorAll('.extra-content a[data-target]');
